...
`\includegraphics{figures/<basename>}` — never absolute. Save every figure inside `$RUN/figures/` (i.e., `output/cap-paper-writer/<slug>/latest/paper/figures/`) so the basename resolves.
